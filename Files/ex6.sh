...
highschool=$(cat wages.csv | cut -d , -f 3,4 |tr "," " " | grep -w 12 |sort -n -k 2| head -n 1 |cut -d " " -f 2) 
collegeschool=$(cat wages.csv | cut -d , -f 3,4 |tr "," " " | grep -w 16 |sort -n -k 2| head -n 1 |cut -d " " -f 2)
echo "$collegeschool - $highschool" | bc
